...
# Direct static files serving disabled for security. Videos must be accessed via HMAC-signed public URLs.
...
# ── Secure Public Videos Distribution ──────────────────────────────────────────

@app.get("/videos/public")
async def serve_public_video(
    file: str,
    signature: str
):
    """
    Serves generated videos only if the URL contains a valid HMAC signature generated by our server.
    Supports partial range requests (206) for smooth seeking/streaming.
    """
    # 1. Verify URL HMAC Signature
    if not verify_url_signature(file, signature):
        raise HTTPException(status_code=403, detail="Access Denied: Invalid URL signature.")

    # 2. Check File Existence & Prevent Directory Traversal
    video_path = os.path.join("generated_videos", file)
    
    # Resolve absolute paths to ensure target is within the generated_videos directory
    abs_base = os.path.abspath("generated_videos")
    abs_target = os.path.abspath(video_path)
    if not abs_target.startswith(abs_base):
        raise HTTPException(status_code=403, detail="Access Denied: Path traversal attempt blocked.")
        
    if not os.path.exists(video_path):
        raise HTTPException(status_code=404, detail="Requested official communication file not found.")

    # 3. Serve via partial contents (supports scrubbing/streaming inline in the browser)
    return FileResponse(
        video_path,
...
async def generate_video(
    ...
    try:
        ...
        ledger_block = ledger.append_block(session_id, video_hash, signature_hex)
        print(f"[PIPELINE] Signature added to ledger. Hash: {video_hash} | Block: #{ledger_block['block_index']}")

        # Generate dynamic, HMAC-signed public URL
