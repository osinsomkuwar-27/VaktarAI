...
void main(){
  ...
  col=mix(vec3(.02, .07, .10),col,min(time*.1,1.));
  col=clamp(col,.02,1.);
  O=vec4(col,1);
}`;
...
class Renderer {
  ...
  private color: [number, number, number] = [0.11, 0.33, 0.43]; // #1D546D default
  ...
  render(now = 0) {
    ...
    gl.clearColor(0.02, 0.07, 0.10, 1); // #061E29 — deep navy
...
// --- FIXED: interface name now matches what the component uses ---
interface SmokeBackgroundProps {
  smokeColor?: string;
}

export const SmokeBackground: React.FC<SmokeBackgroundProps> = ({
  smokeColor = "#1D546D" // #1D546D — mid teal from your palette
}) => {
